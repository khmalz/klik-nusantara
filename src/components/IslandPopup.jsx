...
export default function IslandPopup({ islandName, imageSrc, loading, onClose, onPrev, onNext }) {
   return (
      <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[2000] transition-opacity duration-300">
         <div className="bg-white rounded-xl shadow-xl flex flex-col md:flex-row overflow-hidden w-[95%] max-w-[90vw]">
            <div className="flex flex-col items-center bg-gray-50 p-4 flex-1">
               <div className="relative w-full flex justify-center items-center min-h-[320px] md:min-h-[420px] bg-gray-100">
                  ...
                  <div className="relative z-10 pointer-events-none select-none max-h-[360px]">
                     <IslandImage src={imageSrc} alt={islandName} isLoading={loading} />
                  </div>
               ...
               <button onClick={onClose} className="mt-4 z-20 bg-gray-800 text-white rounded-md px-4 py-2 hover:bg-gray-700 focus:outline-none">
                  ⬅ Back
               </button>
            </div>

            <div className="flex-1 p-6">
               <h2 className="text-2xl font-bold mb-2">{islandName}</h2>
               <p className="text-gray-700 mb-3">Informasi tentang {islandName}. Konten bisa disesuaikan nanti.</p>
